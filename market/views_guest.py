...
def register_with_order(request):
    ...
    try:
        ...
        print(f"📝 Datos recibidos: email={email}, password={'***' if password else None}, username={request.data.get('username')}, order_id={order_id}")
        
        # Validaciones
        if not email or not password:
            print(f"❌ Validación fallida: email={email}, password={'presente' if password else 'ausente'}")
            return Response({
                'success': False,
